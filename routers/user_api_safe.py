...
def sanitize_language(language: str) -> str:
    ...
    if value not in LANGUAGE_CHOICES:
        raise HTTPException(status_code=400, detail='语言仅支持 zh-CN 或 en-US')
    return value
...
async def get_user_dashboard(current_user: str = Depends(get_current_user)):
    ...
    translated_profile = await translate_texts_to_english(
        [risk_preference, trading_style, ai_notes],
        domain='user_profile',
    )
    radar_categories_en = await translate_texts_to_english(
        [item.get('category', '') for item in radar_chart],
        domain='radar_category',
    )
    radar_chart = [
        {
        ...
        for item, category_en in zip(radar_chart, radar_categories_en)
    ]
    ...
    return {
        ...
        'ai_profile': {
            ...
            'risk_preference_en': translated_profile[0],
            'trading_style': trading_style,
            'trading_style_en': translated_profile[1],
            'ai_notes': ai_notes,
            'ai_notes_en': translated_profile[2],
        ...
        'settings': {
            ...
            'language': profile_doc.get('language', 'zh-CN'),
            'tushare_token': None,
        },
    }
...
@router.post('/settings/language', summary='更新界面语言')
async def update_language(request: UpdateLanguageRequest, current_user: str = Depends(get_current_user)):
    language = sanitize_language(request.language)
    await mongo_manager.db['user_profile'].update_one(
        {'user_id': current_user},
        {'$set': sanitize_mongo_document({'language': language, 'updated_at': datetime.datetime.utcnow()})},
        upsert=True,
    )
